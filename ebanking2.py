...
while x == "yes" or "Yes":
    def get_crypto(coin):
        ...
        url = f'https://api.coingecko.com/api/v3/simple/price?ids={coin}&vs_currencies=usd&include_market_cap=false&include_24hr_change=true'
        ...
        price = json.loads(response.text)[coin]['usd']
        change = round(json.loads(response.text)[coin]['usd_24h_change'],2)

        final = str(price) + "(" + str(change) + "%)"
        #price = cg.get_price(ids=coin, vs_currencies='usd', include_24h_change='true')[coin]['usd']
    ...
    3. Check dogecoin's latest price'''))
        if buy_info == 1:
            ...
            print(get_crypto('ethereum'))

        elif buy_info == 3:
            print(get_crypto('dogecoin4'))
